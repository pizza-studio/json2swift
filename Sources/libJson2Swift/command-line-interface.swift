...
public func run(with arguments: [String]) -> ErrorMessage? {
    ...
    else { return "No such file or directory exists." }
    ...
    for jsonFilePath in jsonFilePaths {
        if let errorMessage = generateSwiftFileBasedOnJSON(
            inFile: jsonFilePath,
            includeJSONUtilities: shouldIncludeUtils,
    ...
    if let jsonUtilitiesFilePath = jsonUtilitiesFilePath {
        ...
        else { return "Unable to write JSON utilities file to \(jsonUtilitiesFilePath)" }
    }
...
private func findJSONFileNames(in directory: String) -> [String]? {
    ...
    do { return try FileManager.default.contentsOfDirectory(atPath: directory).filter(isJSONFile) } catch { return nil }
}

private func resolveAbsolutePaths(for jsonFileNames: [String], inDirectory directory: String) -> [String] {
    jsonFileNames.map { (directory as NSString).appendingPathComponent($0) }
}

// MARK: - Generating Swift file based on JSON

private func generateSwiftFileBasedOnJSON(
    inFile jsonFilePath: String,
    includeJSONUtilities: Bool,
    rootTypeName: String
)
    -> ErrorMessage? {
    let url = URL(fileURLWithPath: jsonFilePath)
    let data: Data
    do { data = try Data(contentsOf: url) } catch { return "Unable to read file: \(jsonFilePath)" }

    let jsonObject: Any
    do { jsonObject = try JSONSerialization.jsonObject(with: data, options: []) } catch {
        return "File does not contain valid JSON: \(jsonFilePath)"
    }

    let jsonSchema: JSONElementSchema
    if let jsonElement = jsonObject as? JSONElement {
        jsonSchema = JSONElementSchema.inferred(from: jsonElement, named: rootTypeName)
    } else if let jsonArray = jsonObject as? [JSONElement] {
        jsonSchema = JSONElementSchema.inferred(from: jsonArray, named: rootTypeName)
