...
extension JSONElementSchema {
    ...
    private static func merge(
        ...
        withSchema otherSchema: JSONElementSchema
    )
        -> JSONElementSchema {
...
extension JSONType {
    fileprivate static func merge(
        attributes: JSONAttributeMap,
        with otherAttributes: JSONAttributeMap
    )
        -> JSONAttributeMap {
    ...
    public func findCompatibleType(with type: JSONType) -> JSONType {
        JSONType.compatible(with: self, and: type)
    }

    private static func compatible(with type1: JSONType, and type2: JSONType) -> JSONType {
        switch (type1, type2) {
        // Element
        case let (.element(r1, s1), .element(r2, s2)): return .element(
                isRequired: r1 && r2,
                schema: s1.merged(with: s2)
            )
        ...
        // Element Array
        case let (.elementArray(r1, s1, n1), .elementArray(r2, s2, n2)): return .elementArray(
                isRequired: r1 && r2,
                elementSchema: s1.merged(with: s2),
                hasNullableElements: n1 || n2
            )
        case let (.elementArray(_, s, n), .nullable): return .elementArray(
                isRequired: false,
                elementSchema: s,
                hasNullableElements: n
            )
        case let (.nullable, .elementArray(_, s, n)): return .elementArray(
                isRequired: false,
                elementSchema: s,
                hasNullableElements: n
            )
        ...
        // Value Array
        case let (.valueArray(r1, t1), .valueArray(r2, t2)): return .valueArray(
                isRequired: r1 && r2,
                valueType: t1.findCompatibleType(with: t2)
            )
