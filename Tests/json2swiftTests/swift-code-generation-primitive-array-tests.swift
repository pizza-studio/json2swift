...
class swift_code_generation_primitive_array_tests: XCTestCase {
    ...
    func test_array_of_required_any() {
        let transformation = TransformationFromJSON.toPrimitiveValueArray(
            attributeName: "a",
            propertyName: "p",
            elementType: .any,
    ...
    func test_array_of_required_date() {
        let transformation = TransformationFromJSON.toPrimitiveValueArray(
            attributeName: "a",
            propertyName: "p",
            elementType: .date(format: "M/d/yyyy"),
            hasOptionalElements: false
        )
        XCTAssertEqual(
            transformation.letStatement,
            "let p = (json[\"a\"] as? [String]).flatMap({ $0.toDateArray(withFormat: \"M/d/yyyy\") })"
        )
    }
    ...
    func test_array_of_optional_any() {
        ...
        XCTAssertEqual(
            transformation.letStatement,
            "let p = (json[\"a\"] as? [Any]).map({ $0.toOptionalValueArray() as [Any?] })"
        )
    }
    ...
    func test_array_of_optional_bool() {
        ...
        XCTAssertEqual(
            transformation.letStatement,
            "let p = (json[\"a\"] as? [Any]).map({ $0.toOptionalValueArray() as [Bool?] })"
        )
    }
    ...
    func test_array_of_optional_date() {
        ...
        XCTAssertEqual(
            transformation.letStatement,
            "let p = (json[\"a\"] as? [Any]).map({ $0.toOptionalDateArray(withFormat: \"M/d/yyyy\") })"
        )
    }
    ...
    func test_array_of_optional_double() {
        ...
        XCTAssertEqual(
            transformation.letStatement,
            "let p = (json[\"a\"] as? [Any]).map({ $0.toOptionalDoubleArray() })"
        )
    }
    ...
    func test_array_of_optional_int() {
        ...
        XCTAssertEqual(
            transformation.letStatement,
            "let p = (json[\"a\"] as? [Any]).map({ $0.toOptionalValueArray() as [Int?] })"
        )
    }
    ...
    func test_array_of_optional_string() {
        ...
        XCTAssertEqual(
            transformation.letStatement,
            "let p = (json[\"a\"] as? [Any]).map({ $0.toOptionalValueArray() as [String?] })"
        )
    }
